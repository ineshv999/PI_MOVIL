...
export default function RegistrarUsuarioScreen({ navigation }) {
  ...
  const save = async () => { if (!validate()) return; try { setSaving(true); const parts = form.nombre.trim().split(/\s+/); const first = parts.shift();
    const username = form.correo.trim().toLowerCase();
...
function Field({ label, value, onChange, error, placeholder, keyboardType, secure, box }) { const [visible, setVisible] = useState(false); return <View style={[styles.field, box && styles.half]}><Text style={styles.fieldLabel}>{label}</Text><View><TextInput style={[styles.input, secure && styles.secureInput, error && styles.invalid]} value={value} onChangeText={onChange} placeholder={placeholder} placeholderTextColor={colors.placeholder} keyboardType={keyboardType} secureTextEntry={secure && !visible} autoCapitalize={keyboardType === 'email-address' ? 'none' : 'sentences'} />{secure && <TouchableOpacity style={styles.eye} onPress={() => setVisible((v) => !v)}><Ionicons name={visible ? 'eye-off-outline' : 'eye-outline'} size={19} color={colors.textSecondary} /></TouchableOpacity>}</View>{error && <Text style={styles.error}>{error}</Text>}</View>; }
...
const styles = StyleSheet.create({ content: { padding: 20, paddingBottom: 45 }, section: { color: colors.textPrimary, fontSize: 16, fontWeight: '900', marginBottom: 14 }, optional: { color: colors.textSecondary, fontSize: 11, fontWeight: '500' },
  ...
  input: { minHeight: 48, borderWidth: 1, borderColor: colors.border, borderRadius: 9, backgroundColor: colors.background, color: colors.textPrimary, paddingHorizontal: 12 }, secureInput: { paddingRight: 42 }, eye: { position: 'absolute', right: 12, top: 14 }, invalid: { borderColor: colors.danger }, error: { color: colors.danger, fontSize: 10, fontWeight: '700', marginTop: 5 },
