...
export default function RegistrarActivoScreen({ navigation }) {
  ...
  const [created, setCreated] = useState(null); const [qrData, setQrData] = useState(null); const [qrError, setQrError] = useState(null); const [saving, setSaving] = useState(false);
  ...
  const validate = () => { const e = {}; if (form.nombre.trim().length < 2) e.nombre = 'El nombre del activo es requerido'; if (!form.edificioId) e.edificioId = 'Selecciona un edificio';
    ...
    if (!/^\d+$/.test(form.garantiaCantidad) || +form.garantiaCantidad < 1) e.garantiaCantidad = 'Ingresa una cantidad valida'; if (!form.garantiaPeriodo) e.garantiaPeriodo = 'Selecciona un periodo'; if (photo?.fileSize > 5 * 1024 * 1024) e.photo = 'La fotografia no puede superar 5 MB'; setErrors(e); return !Object.keys(e).length; };
  const pick = async () => { const p = await ImagePicker.requestMediaLibraryPermissionsAsync(); if (!p.granted) return; const r = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ['images'], quality: .75 }); if (!r.canceled) { const item=r.assets[0]; if(item.fileSize>5*1024*1024){setErrors((e)=>({...e,photo:'La fotografia no puede superar 5 MB'}));return;} setPhoto(item);setErrors((e)=>({...e,photo:null})); } };
  ...
  const save = async () => {
    if (!validate()) return;
    setSaving(true); setQrError(null); setQrData(null);
    try {
      const statuses = await endpoints.statuses();
      const status = statuses.find((s) => s.nombre === 'Bueno') || statuses[0];
      if (!status) throw new Error('No hay estatus disponibles para registrar el activo.');
      const asset = await endpoints.createAsset({ codigo_qr: null, nombre: form.nombre.trim(), descripcion: form.observaciones.trim() || null, numero_serie: null,
        ...
        garantia: `${form.garantiaCantidad} ${form.garantiaPeriodo}`, foto_url: null });
      setCreated(asset); setForm(empty); setPhoto(null);
      if (photo) {
        try { await endpoints.uploadAssetPhoto(asset.id, await fileForm(photo)); }
        catch (photoError) { setQrError(`El activo se registro, pero la fotografia no pudo guardarse. ${apiErrorMessage(photoError)}`); }
      }
      try { setQrData(await blobDataUrl(await downloadWithAuth(`/activos/${asset.id}/qr`))); }
      catch (downloadError) { setQrError((current) => current || `El activo se registro, pero no fue posible descargar el QR. ${apiErrorMessage(downloadError)}`); }
    } catch (error) {
      setErrors({ general: `No fue posible registrar el activo. ${apiErrorMessage(error)}` });
  ...
  return <AppShell navigation={navigation} title="Registrar activo" activeRoute="RegistrarActivo"><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    <Card><Text style={styles.section}>Informacion del activo</Text><Field label="NOMBRE DEL ACTIVO *" value={form.nombre} onChange={(v) => change('nombre',v)} error={errors.nombre} placeholder="Ej. Monitor Dell UltraSharp" />
      ...
      <Text style={styles.label}>FOTOGRAFIA (OPCIONAL)</Text><TouchableOpacity style={[styles.photoBox,errors.photo&&styles.invalid]} onPress={pick}>{photo ? <Image source={{uri:photo.uri}} style={styles.photo}/> : <><Ionicons name="image-outline" size={35} color={colors.placeholder}/><Text style={styles.hint}>PNG, JPG, WEBP · max. 5 MB</Text></>}</TouchableOpacity>{errors.photo&&<Text style={styles.error}>{errors.photo}</Text>}
  ...
  <Modal visible={!!created} transparent animationType="fade"><View style={styles.overlay}><View style={styles.modal}><Text style={styles.success}>● REGISTRO EXITOSO</Text><Text style={styles.modalTitle}>¡Activo registrado!</Text><Text style={styles.folio}>{created?.folio}</Text>
    {qrData ? <Image source={{uri:qrData}} style={styles.qr}/> : null}
    {qrError ? <Text style={styles.qrError}>{qrError}</Text> : null}
    <Text style={styles.notice}>El QR es unico para este activo y no cambia aunque edites su informacion.</Text>
    {qrData ? <PrimaryButton title="Descargar QR" icon="download-outline" onPress={download}/> : null}
    {qrData ? <SecondaryButton title="Imprimir" icon="print-outline" onPress={printQr}/> : null}
    <SecondaryButton title="Cerrar" icon="close-outline" onPress={()=>{setCreated(null);setQrError(null);navigation.navigate('InventarioGeneral');}}/></View></View></Modal>
  </AppShell>;
}
